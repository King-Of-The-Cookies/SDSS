...
cdnow.train$gross.profit <- (cdnow.train$rev*0.3)
cdnow.train$weeknr <- as.numeric(sapply(cdnow.train$date, strftime, format = "%V"))
weeklycm <- tapply(cdnow.train$rev, list(cdnow.train$cust, cdnow.train$weeknr), sum)
weeklycm[is.na(test)] <-0
weeklycm <- (test-2)/7

weeklycm <- rowSums(test)/39

cdnow.train["1516",]
cms <- c()
for(i in 1:30){
  discountedCM <- bgnbd.PAlive(params, x,t.x, T.cal) * weeklycm[1516]*((1/(1.002691))^i)
  cms <- c(cms, discountedCM)
}

marketingcostsTotal <- c()
for(i in 1:30){
  discountedMarketingCosts <- 2*((1/(1.002691))^i)
  marketingcostsTotal <- c(marketingcostsTotal, discountedMarketingCosts)
}

sprintf("Should we spend advertising money on this customer?: %s because the NPV of ECM is %s and the discounted marketing costs are %s",ifelse(sum(cms)>sum(marketingcostsTotal),"Yes","No"), sum(cms),sum(marketingcostsTotal))
```
